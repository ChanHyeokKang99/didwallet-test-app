...
import { SAMPLE_TICKETS } from "./home";
...
export default function TicketQRScreen() {
  ...
  const [timeLeft, setTimeLeft] = useState(180); // 3분(180초) 유효시간
  ...
  useEffect(() => {
    const loadTicket = async () => {
      try {
        // 실제로는 API 호출
        setTimeout(() => {
          const foundTicket = SAMPLE_TICKETS.find((t: Ticket) => t.id === id);
          if (foundTicket) {
      ...
      <ThemedView style={[styles.container, { backgroundColor }]}>
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.notFoundContainer}>
            ...
            <AuthButton title="돌아가기" onPress={() => router.back()} />
          </View>
        </SafeAreaView>
      </ThemedView>
    );
